...
def context_key(config: Mapping[str, Any]) -> DropboxContextKey:
    ...
    if not isinstance(team_context, Mapping) or not isinstance(path_root, Mapping):
        raise DropboxContextError("team_context and path_root must be objects.")
...
def _apply_path_root(client: Any, config: Mapping[str, Any]) -> Any:
    ...
    if mode == "namespace_id":
        namespace_id = _required_string(path_root, "namespace_id")
...
def _effective_namespace_id(client: Any, mode: str) -> str:
    ...
    if not isinstance(namespace_id, str) or not namespace_id:
        raise DropboxContextError(f"Dropbox account has no {mode} namespace ID.")
    return namespace_id
