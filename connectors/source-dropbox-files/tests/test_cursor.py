...
def test_cursor_advance_serializes_v3_cursor() -> None:
    cursor = _cursor()
    cursor.add_file(_file())
    cursor.advance_cursor("cursor-final")

    assert cursor.get_state()["cursor"] == "cursor-final"
    assert cursor.has_dropbox_cursor is True


def test_v2_state_upgrades_after_snapshot_cursor_is_advanced() -> None:
    cursor = _cursor()
    ...
    assert cursor.has_dropbox_cursor is False
    cursor.advance_cursor("cursor-v3")
...
@pytest.mark.parametrize(
    ...
    [
        {"version": 3, "files": {}},
        {"version": 3, "scope": {"path": "/Exports", "recursive": True}, "files": {}},
        {"version": 1, "files": []},
...
def test_delta_transfers_new_and_changed_files_without_full_inventory_delete() -> None:
    cursor = _cursor()
    cursor.set_initial_state(
        {
            "version": 3,
            ...
            "cursor": "cursor-1",
            "files": {
                "id:changed": {"path": "changed.pdf", "rev": "r1", "content_hash": "h1"},
                "id:absent-from-delta": {
                    "path": "old.pdf",
                    "rev": "r1",
                    "content_hash": "h1",
                },
    ...
    changed = _file(file_id="id:changed", path="changed.pdf", rev="r2", content_hash="h2")
    new = _file(file_id="id:new", path="new.pdf", rev="r1", content_hash="h1")

    plan = cursor.plan_delta(
        [changed, new],
        [],
        rename_policy="ignore",
        delete_policy="delete",
    ...
    assert plan.operations == []
    assert plan.files == [changed, new]


def test_delta_propagates_rename_and_delete_tombstone() -> None:
    cursor = _cursor()
    cursor.set_initial_state(
        {
            "version": 3,
...
def test_delta_rename_tombstone_for_same_file_is_not_a_delete() -> None:
    cursor = _cursor()
    cursor.set_initial_state(
        {
            "version": 3,
    ...
    renamed = _file(file_id="id:file", path="new.pdf", rev="r1", content_hash="h1")
...
def test_delta_changed_rename_tombstone_emits_move_and_file_without_delete() -> None:
    cursor = _cursor()
    cursor.set_initial_state(
        {
            "version": 3,
    ...
    renamed = _file(file_id="id:file", path="new.pdf", rev="r2", content_hash="h2")
    ...
    assert plan.files == [renamed]
    cursor.mark_move(plan.operations[0])
    cursor.add_file(renamed)
    assert cursor.get_state()["files"]["id:file"] == {
        "path": "new.pdf",
        "rev": "r2",
        "content_hash": "h2",
...
def test_delta_new_file_does_not_consume_unrelated_delete_tombstone() -> None:
    cursor = _cursor()
    cursor.set_initial_state(
        {
            "version": 3,
    ...
    new = _file(file_id="id:new", path="new.pdf", rev="r1", content_hash="h2")
...
def test_delta_ignored_rename_tombstone_is_not_a_delete() -> None:
    cursor = _cursor()
    cursor.set_initial_state(
        {
            "version": 3,
...
def test_delta_renamed_changed_file_uses_pinned_path_when_rename_is_ignored() -> None:
    cursor = _cursor()
    cursor.set_initial_state(
        {
            "version": 3,
    ...
    assert plan.operations == []
    assert plan.files[0].uri == "old.pdf"
...
def test_successful_slice_advances_dropbox_cursor(monkeypatch: pytest.MonkeyPatch) -> None:
    cursor = _cursor()
    cursor.set_initial_state(
        {
            "version": 3,
    ...
    assert list(
        stream.read_records_from_slice(
            {"operations": [], "files": [], "dropbox_cursor": "cursor-2"}
        )
    ) == []
    assert cursor.get_state()["cursor"] == "cursor-2"


def test_file_staging_error_withholds_dropbox_cursor(monkeypatch: pytest.MonkeyPatch) -> None:
    cursor = _cursor()
    cursor.set_initial_state(
        {
            "version": 3,
    ...
    stream.config = FileBasedStreamConfig(name="raw_files", format=UnstructuredFormat())
    stream._cursor = cursor
    error = AirbyteMessage(
        type=Type.LOG,
        log=AirbyteLogMessage(level=Level.ERROR, message="file failed"),
    )
    monkeypatch.setattr(
        "source_dropbox_files.source.DefaultFileBasedStream.read_records_from_slice",
        lambda *_args: iter([error]),
    )
    ...
    assert cursor.get_state()["cursor"] == "cursor-1"


def test_compute_slices_uses_delta_cursor_for_move_and_delete() -> None:
    cursor = _cursor()
    cursor.set_initial_state(
        {
            "version": 3,
    ...
    reader = SimpleNamespace(
        ...
        iter_delta_entries=lambda cursor: iter(
            [
                FileMetadata(
                    name="new.pdf",
                    id="id:renamed",
                    path_lower="/exports/new.pdf",
                    path_display="/Exports/new.pdf",
                    rev="012345678",
                    size=1,
                    content_hash="0" * 64,
                ),
                DeletedMetadata(
                    name="gone.pdf",
                    path_lower="/exports/gone.pdf",
                    path_display="/Exports/gone.pdf",
                ),
            ]
        ),
        _remote_file=lambda _entry: _file(
            file_id="id:renamed", path="new.pdf", rev="r1", content_hash="h1"
        ),
        _max_file_size_bytes=1024,
        file_matches_globs=lambda _file, _globs: True,
        _relative_path=lambda path: path.removeprefix("/Exports/"),
    )
    stream.stream_reader = reader

    slices = stream.compute_slices()

    assert slices[0]["dropbox_cursor"] == "cursor-2"
